          ...
          Wrap(
            ...
            children: [
              TextButton.icon(
                ...
                label: Text(DateFormat.yMMMd().format(value.start)),
                onPressed: () async {
                  ...
                  onChanged(
                    value.copyWith(
                      ...
                      clearEnd:
                          value.end != null &&
                          RepeatSchedule.day(value.end!).isBefore(date),
                    ),
                  );
                },
              ),
              TextButton.icon(
                ...
                label: Text(
                  TimeOfDay.fromDateTime(value.start).format(context),
                ),
                onPressed: () async {
                  final time = await showTimePicker(
                    context: context,
                    initialTime: TimeOfDay.fromDateTime(value.start),
                  );
                ...
                Expanded(
                  child: TextButton.icon(
                    ...
                    label: Text(
                      value.end == null
                          ? 'Ends: never'
                          : 'Ends: ${DateFormat.yMMMd().format(value.end!)}',
                    ),
                    onPressed: () async {
                      final date = await showDatePicker(
                        context: context,
                        initialDate: value.end ?? value.start,
