...
class CalendarRepository {
  ...
  Stream<List<Task>> watchTasksWithDueDate() {
    return (_db.select(_db.tasks)..where((t) => t.dueDate.isNotNull())).watch();
  }
  ...
  Future<RepeatSchedule?> getSeriesSchedule(Event event) async {
    final head = event.recurrenceId == null
        ? event
        : await getEvent(event.recurrenceId!);
  ...
  Future<void> updateFollowingEvents({
    ...
    if (RepeatSchedule.day(
      startTime,
    ).isBefore(RepeatSchedule.day(selected.startTime))) {
      throw ArgumentError(
        'The replacement series cannot start before the selected date',
      );
    }
    ...
    final originalWeekdays = originalRule == null
        ? <int>{}
        : (originalRule.weekdays.isEmpty
                  ? [originalRule.start.weekday]
                  : originalRule.weekdays)
              .toSet();
    final newWeekdays =
    ...
    final sameWeekdays =
        ...
        (originalWeekdays.length == newWeekdays.length &&
            originalWeekdays.containsAll(newWeekdays));
    final preserveDates =
        originalRule != null &&
        schedule.frequency == originalRule.frequency &&
        sameWeekdays &&
        schedule.end == originalRule.end &&
        RepeatSchedule.day(startTime) == RepeatSchedule.day(selected.startTime);
    final cutoff = DateTime(
      selected.startTime.year,
      selected.startTime.month,
      selected.startTime.day - 1,
    );
    if (head != null && head.startTime.isBefore(selected.startTime)) {
      final rule = (await getSeriesSchedule(head))!;
      final json = head.schedule == null
          ? <String, dynamic>{}
          : jsonDecode(head.schedule!) as Map<String, dynamic>;
      json.addAll(
        jsonDecode(rule.copyWith(end: cutoff).encode()) as Map<String, dynamic>,
      );
      await (_db.update(_db.events)..where((e) => e.id.equals(head.id))).write(
        EventsCompanion(
          recurrenceEndDate: Value(cutoff),
          schedule: Value(jsonEncode(json)),
        ),
      );
    }
    if (preserveDates) {
      ...
      for (final event in following) {
        ...
        await (_db.update(
          ...
          EventsCompanion(
            ...
            recurrenceId: Value(selected.id),
            schedule: Value(encoded),
            ...
            recurrenceNextGenerationDate: Value(
              event.id == selected.id
                  ? head?.recurrenceNextGenerationDate ?? last
                  : null,
            ),
    ...
    await (_db.delete(_db.events)..where(
          (e) =>
              e.recurrenceId.equals(seriesId) &
              e.startTime.isBiggerOrEqualValue(selected.startTime),
        ))
        .go();
    await createEvent(
      title: title,
  ...
  Future<void> updateEvent({
    ...
    if (rule != null) {
      ...
      final first = rule
          .between(
            rule.start,
            DateTime(rule.start.year + 1, rule.start.month, rule.start.day),
          )
          .firstOrNull;
      if (first == null) throw ArgumentError('No scheduled date in this range');
      final duration = endTime?.difference(startTime);
    ...
    await (_db.update(_db.events)..where((e) => e.id.equals(id))).write(
      EventsCompanion(
        ...
        recurrenceEndDate: rule == null
            ? const Value.absent()
            : Value(rule.end),
        recurrenceId: rule != null && rule.frequency != 'none'
            ? Value(id)
            : const Value.absent(),
        reminderEnabled: Value(reminderEnabled),
        reminderMode: Value(reminderMode.storageValue),
  ...
  Future<void> extendRecurringEvents() => _db.transaction(() async {
    final heads =
        await (_db.select(_db.events)..where(
              (e) =>
                  e.recurrenceId.equalsExp(e.id) &
                  e.frequency.equals('none').not(),
            ))
            .get();

    final horizonEnd = DateTime.now().add(
    ...
    for (final head in heads) {
      ...
      if (head.recurrenceEndDate != null &&
          !generatedUpTo.isBefore(head.recurrenceEndDate!)) {
        continue;
      }
  ...
  Future<void> _generateOccurrences(
    Event head, {
    ...
    final cutoff = head.recurrenceEndDate == null
        ? horizonEnd
        : (head.recurrenceEndDate!.isBefore(horizonEnd)
              ? head.recurrenceEndDate!
              : horizonEnd);
    if (!RepeatSchedule.day(from).isBefore(RepeatSchedule.day(cutoff))) return;
    final template = head.schedule == null
        ? null
        : (jsonDecode(head.schedule!) as Map<String, dynamic>)['template']
              as Map<String, dynamic>?;
    final duration = template == null
        ? head.endTime?.difference(head.startTime)
        : template['durationMinutes'] == null
        ? null
        : Duration(minutes: template['durationMinutes'] as int);

    final rule =
        RepeatSchedule.decode(head.schedule) ??
        RepeatSchedule(
          start: head.startTime,
          frequency: head.frequency,
          end: head.recurrenceEndDate,
        );
    for (final next in rule.between(
          ...
          .insert(
            EventsCompanion.insert(
              ...
              reminderEnabled: Value(
                template?['reminderEnabled'] as bool? ?? head.reminderEnabled,
              ),
              reminderMode: Value(
                template?['reminderMode'] as String? ?? head.reminderMode,
              ),
              reminderMinutesBefore: Value(
                template?['reminderMinutesBefore'] as int? ??
                    head.reminderMinutesBefore,
              ),
            ),
          );
    }
