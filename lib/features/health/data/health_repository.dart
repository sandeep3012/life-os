...
class HealthRepository {
  ...
  Future<void> setTaken(String medicationId, DateTime date, bool taken) async {
    ...
    final existing =
        await (_db.select(_db.medicationLogs)..where(
              (l) => l.medicationId.equals(medicationId) & l.date.equals(day),
            ))
            .getSingleOrNull();
  ...
  Future<void> _adjustStock(String medicationId, int delta) async {
    final med = await (_db.select(
      _db.medications,
    )..where((m) => m.id.equals(medicationId))).getSingleOrNull();
  ...
  Stream<List<Exercise>> watchExercises() {
    return (_db.select(
      _db.exercises,
    )..orderBy([(e) => OrderingTerm.asc(e.position)])).watch();
  }
  ...
  Future<void> updateExercise({
    ...
    return (_db.update(_db.exercises)..where((e) => e.id.equals(id))).write(
      ExercisesCompanion(name: Value(name), scheme: Value(scheme)),
    );
  }
  ...
  Future<void> deleteWorkoutDay(String id) async {
    final own = await (_db.select(
      _db.exercises,
    )..where((e) => e.workoutDayId.equals(id))).get();
  ...
  Future<void> deleteExercise(String id) async {
    await (_db.delete(
      _db.exerciseSetLogs,
    )..where((l) => l.exerciseId.equals(id))).go();
    await (_db.delete(
      _db.workoutLogs,
    )..where((l) => l.exerciseId.equals(id))).go();
  ...
  Future<void> logSet({
    ...
    final day = dateOnly(date);
    final existing =
        await (_db.select(_db.exerciseSetLogs)..where(
              (l) => l.exerciseId.equals(exerciseId) & l.date.equals(day),
            ))
            .get();
    final next =
        existing.fold<int>(0, (m, l) => l.setNumber > m ? l.setNumber : m) + 1;

    await _db
        .into(_db.exerciseSetLogs)
        .insert(
  ...
  Future<void> deleteSet(String id) async {
    ...
    final rest =
        await (_db.select(_db.exerciseSetLogs)
              ..where(
                (l) =>
                    l.exerciseId.equals(row.exerciseId) &
                    l.date.equals(row.date),
              )
              ..orderBy([(l) => OrderingTerm.asc(l.setNumber)]))
            .get();
    for (var i = 0; i < rest.length; i++) {
      if (rest[i].setNumber == i + 1) continue;
      await (_db.update(_db.exerciseSetLogs)
            ..where((l) => l.id.equals(rest[i].id)))
