...
void main() {
  ...
  test(
    ...
    () async {
      ...
      container.listen(selectedDayItemsProvider, (_, items) {
        if (items.length == 4 && !ready.isCompleted) ready.complete();
      });
  ...
  test(
    'createEvent with daily frequency and an end date generates occurrences',
    () async {
      final repo = CalendarRepository(db);
  ...
  test(
    'createEvent with monthly frequency and no end date stops at the horizon',
    () async {
      final repo = CalendarRepository(db);
  ...
  test(
    'extendRecurringEvents tops up a stale series and is idempotent',
    () async {
      final repo = CalendarRepository(db);
      final start = DateTime.now();
      final head = await repo.createEvent(
  ...
  test(
    'updateEvent changes title/time/reminder but not recurrence columns',
    () async {
      final repo = CalendarRepository(db);
  ...
  test(
    'deleteEvent on a head row leaves already-generated occurrences intact',
    () async {
      final repo = CalendarRepository(db);
      final start = DateTime.now();
      final head = await repo.createEvent(
  ...
  test(
    'getEventsInSeries returns exactly the rows sharing a recurrenceId',
    () async {
      final repo = CalendarRepository(db);
      final start = DateTime(2026, 3, 10, 9);
      final head = await repo.createEvent(
      ...
      expect(
        inSeries.every((e) => e.recurrenceId == head.recurrenceId),
        isTrue,
      );
  ...
  test(
    'deleteEventSeries removes every row in the series and leaves others untouched',
    () async {
      final repo = CalendarRepository(db);
      final start = DateTime(2026, 3, 10, 9);
      final head = await repo.createEvent(
