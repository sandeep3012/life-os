...
void main() {
  ...
  testWidgets(
    'creating a goal linked to a habit shows it with a 0% ring, and progress can be bumped',
    (tester) async {
      ...
      expect(
        find.text('No goals yet — add one to start tracking progress.'),
        findsOneWidget,
      );
  ...
  testWidgets(
    'setting a deadline in the sheet round-trips to the created goal',
    (tester) async {
      await tester.pumpWidget(buildApp());
      await tester.pumpAndSettle();
