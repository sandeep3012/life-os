...
void main() {
  ...
  Widget buildApp() {
    return UncontrolledProviderScope(
      ...
      child: MaterialApp(
        ...
        home: const AiAnalyserScreen(),
      ),
    );
  }

  testWidgets(
    'refreshing surfaces an overspend insight, dismissing removes it',
    (tester) async {
      ...
      await financeRepo.createBudget(
      ...
      await habitsRepo.createHabit(
        ...
        schedule: RepeatSchedule(
          start: dateOnly(DateTime.now()).subtract(const Duration(days: 7)),
          frequency: 'daily',
        ),
      );
      ...
      await tester.runAsync(
        () => container.read(aiAnalyserControllerProvider).refresh(),
      );
      await tester.pump();
  ...
  testWidgets('refreshing surfaces a bill-due-soon insight', (tester) async {
    await SettingsRepository(db).setCurrencyCode('EUR');
    final financeRepo = FinanceRepository(db, FileStorageService());
  ...
  testWidgets('refreshing surfaces a habit category rollup insight', (
    tester,
  ) async {
    ...
    expect(
      find.textContaining('"Fitness" habits are at risk today'),
      findsOneWidget,
    );
