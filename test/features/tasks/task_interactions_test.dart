...
void main() {
  testWidgets(
    'task body opens details while checkbox only changes completion',
    (tester) async {
      var opened = 0;
      var toggled = 0;
      final task = Task(
  ...
  testWidgets(
    'task reminder is visible, opt-in, and date toggle does not enable it',
    (tester) async {
      await tester.pumpWidget(
        ProviderScope(
          overrides: [
            ...
            home: Scaffold(
              ...
              body: Builder(
                builder: (context) => TextButton(
                  onPressed: () => showQuickAddTaskSheet(context),
  ...
  testWidgets(
    'habit sheet has compact description and visible cancel control',
    (tester) async {
      await tester.pumpWidget(
        ProviderScope(
          child: MaterialApp(
            home: Scaffold(
              body: Builder(
                builder: (context) => TextButton(
                  onPressed: () =>
                      showQuickAddHabitSheet(context, categories: []),
                  child: const Text('Open'),
                ),
              ),
            ),
