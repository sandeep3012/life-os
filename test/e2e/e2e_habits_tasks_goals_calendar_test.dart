...
void main() {
  ...
  group('E2E Habits, Tasks, Goals & Calendar Lifecycle & Edge Cases', () {
    test(
      'habit streaks, missed days, multiple logs, and weekly completion matrix',
      () async {
        ...
        final fourDaysAgo = today.subtract(
          const Duration(days: 4),
        ); // 3 days ago missing -> break
        ...
        final progressAfterUntoggle = container.read(
          habitsWithProgressProvider,
        );
    ...
    test('goals with linked habits and milestone checklist progress', () async {
      ...
      final milestone1Id = await goalsRepo.createMilestone(
        goalId: goalId,
        title: 'Run 10km without stopping',
      );
      final milestone2Id = await goalsRepo.createMilestone(
        goalId: goalId,
        title: 'Run 21km Half Marathon',
      );

      // 3. Link a Habit to the Goal
      final habitId = await habitsRepo.createHabit('Daily 5km Jog');
      await goalsRepo.addLink(
        goalId: goalId,
        linkedType: 'habit',
        linkedId: habitId,
      );
      ...
      final updatedMilestones = await goalsRepo
          .watchMilestonesForGoal(goalId)
          .first;
      expect(
        updatedMilestones.firstWhere((m) => m.id == milestone1Id).completed,
        isTrue,
      );
      expect(
        updatedMilestones.firstWhere((m) => m.id == milestone2Id).completed,
        isFalse,
      );
    ...
    test(
      'polymorphic calendar aggregation across tasks, habits, and manual events without duplication',
      () async {
        final targetDate = dateOnly(DateTime.now());
        ...
        expect(
          dayItems.firstWhere((i) => i.type == CalendarItemType.event).sourceId,
          eventId.id,
        );
        expect(
          dayItems.firstWhere((i) => i.type == CalendarItemType.task).sourceId,
          taskId,
        );
        expect(
          dayItems.firstWhere((i) => i.type == CalendarItemType.habit).sourceId,
          habitId,
        );
        expect(
          dayItems.firstWhere((i) => i.type == CalendarItemType.bill).sourceId,
          bill.id,
        );
        ...
        expect(
          updatedItems.any((i) => i.type == CalendarItemType.task),
          isFalse,
        );
      },
    );
