...
void main() {
  Habit habit(RepeatSchedule schedule) => Habit(
    id: 'h',
    name: 'Read',
    frequency: 'daily',
    targetPerWeek: 7,
  ...
  List<HabitLog> logs(List<int> days) => [
  ...
  test('counts streak runs and keeps pending today intact', () {
    final h = habit(
      RepeatSchedule(start: DateTime(2026, 9, 1), frequency: 'daily'),
    );
    final stats = HabitStatistics.calculate(
      h,
      logs([1, 2, 4, 5]),
      DateTime(2026, 9, 6),
    );
  ...
  test('future habits have zero statistics', () {
    final h = habit(
      RepeatSchedule(start: DateTime(2026, 10, 1), frequency: 'daily'),
    );
