...
void main() {
  ...
  test(
    'task completion and repeated generation preserve fixed dates and totals',
    () async {
  ...
  test(
    'weekly tasks begin on a selected day, not an unselected start day',
    () async {
      final start = RepeatSchedule.day(DateTime.now());
  ...
  test(
    'event description is copied and inclusive end retains final occurrence',
    () async {
  ...
  test(
    'converting an event to selected weekdays uses the first matching day',
    () async {
      final repo = CalendarRepository(db);
      final start = RepeatSchedule.day(DateTime.now());
      final next = DateTime(start.year, start.month, start.day + 1);
  ...
  test(
    'editing following events preserves earlier rows and updates future template',
    () async {
      final repo = CalendarRepository(db);
      final start = RepeatSchedule.day(DateTime.now());
      final end = DateTime(start.year, start.month, start.day + 4);
      ...
      await repo.updateFollowingEvents(
        id: selected.id,
        title: 'Updated',
  ...
  test(
    'changing future frequency truncates the old series without duplicate regeneration',
    () async {
      final repo = CalendarRepository(db);
      final start = RepeatSchedule.day(DateTime.now());
  ...
  test(
    'habit starts and unscheduled dates cannot be marked complete',
    () async {
  ...
  test('legacy JSON without new nullable fields still restores', () async {
    ...
    final json = (await repo.getHabit(id))!.toJson()
      ..remove('schedule')
      ..remove('description');
